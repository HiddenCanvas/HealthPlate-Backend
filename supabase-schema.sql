...
CREATE INDEX IF NOT EXISTS idx_meal_plan_items_date 
  ON meal_plan_items(meal_date); [cite: 137]

CREATE INDEX IF NOT EXISTS idx_meal_plans_activated 
  ON meal_plans(activated_at, expires_at); [cite: 138]
...
CREATE TRIGGER on_auth_user_created
    ...
    FOR EACH ROW EXECUTE PROCEDURE public.handle_new_user(); [cite: 196]
...
CREATE TRIGGER on_user_created_notif_settings
  ...
  FOR EACH ROW EXECUTE PROCEDURE public.handle_new_user_notif_settings(); [cite: 144]
...
ALTER TABLE "users"                 ENABLE ROW LEVEL SECURITY; [cite: 197]
ALTER TABLE "daily_logs"            ENABLE ROW LEVEL SECURITY; [cite: 198]
ALTER TABLE "log_entries"           ENABLE ROW LEVEL SECURITY; [cite: 199]
ALTER TABLE "meal_plans"            ENABLE ROW LEVEL SECURITY; [cite: 200]
ALTER TABLE "meal_plan_items"       ENABLE ROW LEVEL SECURITY; [cite: 200]
ALTER TABLE "recipes"               ENABLE ROW LEVEL SECURITY; [cite: 201]
ALTER TABLE "recipe_steps"          ENABLE ROW LEVEL SECURITY;
ALTER TABLE "bahan_resep"           ENABLE ROW LEVEL SECURITY; [cite: 202]
ALTER TABLE "user_bookmarks"        ENABLE ROW LEVEL SECURITY; [cite: 202]
ALTER TABLE "notifications"         ENABLE ROW LEVEL SECURITY; [cite: 203]
ALTER TABLE "food_products"         ENABLE ROW LEVEL SECURITY; [cite: 203]
ALTER TABLE "food_category"         ENABLE ROW LEVEL SECURITY; [cite: 204]
ALTER TABLE "meal_categories"       ENABLE ROW LEVEL SECURITY;
ALTER TABLE "meal_packages"         ENABLE ROW LEVEL SECURITY;
ALTER TABLE "notification_settings" ENABLE ROW LEVEL SECURITY; [cite: 141]
ALTER TABLE "articles"              ENABLE ROW LEVEL SECURITY; [cite: 157]
ALTER TABLE "article_likes"         ENABLE ROW LEVEL SECURITY; [cite: 158]
ALTER TABLE "article_bookmarks"     ENABLE ROW LEVEL SECURITY; [cite: 158]
ALTER TABLE "tips"                  ENABLE ROW LEVEL SECURITY; [cite: 159]

-- === POLICIES: USERS ===
CREATE POLICY "users_select_all_for_join" ON "users" FOR SELECT USING (true); [cite: 174]
CREATE POLICY "users_update_own" ON "users" FOR UPDATE USING (auth.uid() = user_id); [cite: 205]
...
CREATE POLICY "daily_logs_own" ON "daily_logs" FOR ALL USING (auth.uid() = user_id); [cite: 206]
...
-- === POLICIES: MEAL PLANS ===
CREATE POLICY "meal_plans_own" ON "meal_plans" FOR ALL USING (auth.uid() = user_id); [cite: 208]
...
CREATE POLICY "recipes_select_all" ON "recipes" FOR SELECT USING (true); [cite: 210]
CREATE POLICY "recipes_insert_own" ON "recipes" FOR INSERT WITH CHECK (auth.uid() = user_id OR user_id IS NULL); [cite: 211]
CREATE POLICY "recipes_update_own" ON "recipes" FOR UPDATE USING (auth.uid() = user_id); [cite: 212]
CREATE POLICY "recipes_delete_own" ON "recipes" FOR DELETE USING (auth.uid() = user_id); [cite: 213]
CREATE POLICY "recipes_admin_all" ON "recipes" FOR ALL USING (auth.uid() IN (SELECT user_id FROM users WHERE role = 'admin')); [cite: 6]

CREATE POLICY "bahan_resep_select" ON "bahan_resep" FOR SELECT USING (true); [cite: 214]
...
-- === POLICIES: ARTICLES ===
CREATE POLICY "articles_select_all" ON "articles" FOR SELECT USING (true); [cite: 160]
CREATE POLICY "articles_insert_own" ON "articles" FOR INSERT WITH CHECK (auth.uid() = user_id); [cite: 161]
CREATE POLICY "articles_update_own" ON "articles" FOR UPDATE USING (auth.uid() = user_id); [cite: 162]
CREATE POLICY "articles_delete_own" ON "articles" FOR DELETE USING (auth.uid() = user_id); [cite: 163]
CREATE POLICY "articles_admin_all" ON "articles" FOR ALL USING (auth.uid() IN (SELECT user_id FROM users WHERE role = 'admin')); [cite: 4]

-- === POLICIES: ARTICLE ENGAGEMENTS ===
CREATE POLICY "article_likes_select_all" ON "article_likes" FOR SELECT USING (true); [cite: 164]
CREATE POLICY "article_likes_insert_own" ON "article_likes" FOR INSERT WITH CHECK (auth.uid() = user_id); [cite: 165]
CREATE POLICY "article_likes_delete_own" ON "article_likes" FOR DELETE USING (auth.uid() = user_id); [cite: 166]

CREATE POLICY "article_bookmarks_select_all" ON "article_bookmarks" FOR SELECT USING (true); [cite: 167]
CREATE POLICY "article_bookmarks_insert_own" ON "article_bookmarks" FOR INSERT WITH CHECK (auth.uid() = user_id); [cite: 168]
CREATE POLICY "article_bookmarks_delete_own" ON "article_bookmarks" FOR DELETE USING (auth.uid() = user_id); [cite: 169]

-- === POLICIES: TIPS ===
CREATE POLICY "tips_select_all" ON "tips" FOR SELECT USING (true); [cite: 170]
CREATE POLICY "tips_insert_own" ON "tips" FOR INSERT WITH CHECK (auth.uid() = user_id); [cite: 171]
CREATE POLICY "tips_update_own" ON "tips" FOR UPDATE USING (auth.uid() = user_id); [cite: 172]
CREATE POLICY "tips_delete_own" ON "tips" FOR DELETE USING (auth.uid() = user_id); [cite: 173]
CREATE POLICY "tips_admin_all" ON "tips" FOR ALL USING (auth.uid() IN (SELECT user_id FROM users WHERE role = 'admin')); [cite: 5]

-- === POLICIES: SYSTEM INTERNALS ===
CREATE POLICY "bookmarks_own" ON "user_bookmarks" FOR ALL USING (auth.uid() = user_id); [cite: 216]
CREATE POLICY "notifications_own" ON "notifications" FOR ALL USING (auth.uid() = user_id); [cite: 217]
CREATE POLICY "food_products_select" ON "food_products" FOR SELECT USING (true); [cite: 218]
CREATE POLICY "food_category_select" ON "food_category" FOR SELECT USING (true); [cite: 219]
CREATE POLICY "notif_settings_own" ON "notification_settings" FOR ALL USING (auth.uid() = user_id); [cite: 141]
...
ON CONFLICT DO NOTHING; [cite: 220]
...
ON CONFLICT (barcode_code) DO NOTHING; [cite: 221-222]
...
-- UPDATE users SET role = 'admin' WHERE email = 'admin@healthplate.com'; [cite: 2]